...
:::info TypeScript Compatibility
This suite uses TypeScript's compiler API and supports TypeScript 6. See [TypeScript Compatibility](/docs/presets/tsconfig/compatibility) before upgrading the compiler independently.
:::
...
<Tabs groupId="type-declarations-examples">
  <TabItem value="correct" label="Correct" default>
    ...
    A clean source file plus its mirror that passes all eleven checks: kebab-case path, no identifier equal to the file name, every body var and return typed through a path-derived alias, alphabetical sections, an object property prefixed with its parent type, and full source-to-mirror coverage.
  ...
  <TabItem value="incorrect" label="Incorrect">
    ...
    One source file and its mirror that trip eight of the eleven checks. Each violating line is annotated with the exact toggle key it fails.
...
|-----------------------|------------------------------------------------------------|--------------------------|-------------------------------------------------------------------------------------------------------|
| `vitest`              | `typeof import('vitest')`                                  | required                 | Vitest module namespace used to register the suite.                                                   |
