...
Each policy field is independently optional. When a field is defined, the recipe passes the corresponding flag to `gh repo edit`.

When a field is undefined, that flag is omitted entirely and GitHub's current value is preserved.
