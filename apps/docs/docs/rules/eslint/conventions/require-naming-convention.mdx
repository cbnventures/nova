...
<Tabs groupId="require-naming-convention-examples">
  ...
  <TabItem value="incorrect" label="Incorrect">
    ...
    // Glued PascalCase — type aliases must use UnderscorePascalCase.
    type TestsTypeDeclarationsExtractObjectTypesObjectType = {};
    type myType = string;
    ```
...
The default for `typeAlias` changed from `PascalCase` to `UnderscorePascalCase` in 0.18.0 so type aliases align with the prefixes produced by `require-type-naming`. Existing glued names like `CliUtilityChangelog` now fail; rename them to the underscore form (`Cli_Utility_Changelog`) or override the option back to `PascalCase` to defer the change.
:::
