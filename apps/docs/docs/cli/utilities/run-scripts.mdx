...
Run `package.json` scripts by pattern in sequential or parallel mode with no extra <Terminology title="runtime dep" to="/docs/quickstart/terminology#packages-and-dependencies" color={true}>dependencies</Terminology>.
...
- Run related scripts (e.g., `build:clean`, `build:transpile`, `build:fix-alias`) in a single command with no extra <Terminology title="runtime dep" to="/docs/quickstart/terminology#packages-and-dependencies" color={true}>dependencies</Terminology>.
