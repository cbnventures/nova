...
<Tabs groupId="no-multiline-strings-config">
  ...
  <TabItem value="allow-escape-sequences" label="Allow Escape Sequences">
    ...
    Allow internal `\n` escape sequences in strings (backslash line continuation and visual newlines in template literals are still flagged):
...
- **Multiline tagged template literal is not flagged** — Tagged template expressions (e.g. `` css`...` ``, `` gql`...` ``) are deliberately exempt; the rule only inspects untagged string and template literals, so newlines inside a tagged template are never reported.
