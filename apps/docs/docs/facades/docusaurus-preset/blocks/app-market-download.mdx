...
description: "App store download buttons for Apple App Store and Google Play with per-preset styling and configurable download link targets."
sidebar_position: 7
keywords:
  - "nova"
  - "docusaurus facades"
...
The App Market Download block renders a `<section>` with one or two download buttons. Each button links to the respective app store. The block renders nothing if neither `appStoreUrl` nor `googlePlayUrl` is provided.
