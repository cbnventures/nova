...
- **Already set the `regexFile` option?** — That file is skipped automatically, so you do not also need to list it in `ignoreFiles`. Use `ignoreFiles` only for additional files.
