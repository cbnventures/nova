...
Lint MDX content with the `eslint-mdx` parser and the `eslint-plugin-mdx` remark plugin so your docs and prose are checked under the same <Terminology title="single-file config" to="/docs/quickstart/terminology#builds-and-tooling" color={true}>flat config</Terminology> as your source.
