...
- **No centralized file yet** — Leave `regexFile` unset or empty. The rule stays active and reports inline patterns with destination-neutral guidance.
- **Error fires inside the patterns file** — Check that `regexFile` resolves to that exact file. The configured target is skipped automatically.
- **Need additional exceptions** — Use `ignoreFiles` only for files where the rule does not apply. The configured `regexFile` does not need to be listed again.
- **Inline `new RegExp()` is reported** — Import a shared pattern and pass the reference to the constructor. Static inline string and template patterns are reported.
