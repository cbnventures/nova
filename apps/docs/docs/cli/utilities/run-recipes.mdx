...
id: "run-recipes"
title: "Run Recipes"
description: "Run all configured recipes in sequence across the GitHub, license, read-me, and package-json categories with a single command."
keywords:
  - "nova"
  - "nova run-recipes"
  - "package.json"
...
There is no category flag, so every invocation runs all recipes across the `github`, `license`, `read-me`, and `package-json` categories.

Recipes reconcile settings or existing content and are safe to run again. They do not invoke <Terminology title="template command" to="/docs/quickstart/terminology#nova-concepts" color={true}>generators</Terminology>; generated files remain an explicit `nova generate ...` action.
