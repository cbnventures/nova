...
import type {
  TestsCliGenerateMustHavesReadMeRunBannerExpected,
  TestsCliGenerateMustHavesReadMeRunBannerWritten,
...
describe('CliGenerateMustHavesReadMe.run', async () => {
  ...
  it('includes markdown banner header in generated file', async () => {
    const projectDirectory: TestsCliGenerateMustHavesReadMeRunProjectDirectory = join(sandboxRoot, 'includes-banner');
    ...
    const written: TestsCliGenerateMustHavesReadMeRunBannerWritten = await readFile(readmePath, 'utf-8');

    const expectedBanner: TestsCliGenerateMustHavesReadMeRunBannerExpected = [
      '<!--',
    ...
    ok(written.startsWith(expectedBanner), 'Expected README.md to start with the generated file banner');
