...
export class Runner {
  ...
  public static async run(options: Cli_Recipe_PackageJson_SyncEnvironment_Runner_Run_Options): Cli_Recipe_PackageJson_SyncEnvironment_Runner_Run_Returns {
    ...
    const eligibleWorkspaces: Cli_Recipe_PackageJson_SyncEnvironment_Runner_Run_EligibleWorkspaces = workingFileWorkspaces.filter((workspace) => {
      const workspaceConfig: Cli_Recipe_PackageJson_SyncEnvironment_Runner_Run_WorkspaceConfigFilter = workspace[1];
      const workspaceRecipes: Cli_Recipe_PackageJson_SyncEnvironment_Runner_Run_WorkspaceRecipesFilter = workspaceConfig['recipes'];

      if (workspaceRecipes === undefined) {
        return false;
      }

      const recipeTuple: Cli_Recipe_PackageJson_SyncEnvironment_Runner_Run_RecipeTupleFilter = workspaceRecipes['sync-environment'];
