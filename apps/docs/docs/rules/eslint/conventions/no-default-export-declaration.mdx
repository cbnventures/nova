...
description: "Disallow attaching function or class declarations to export default so every exported symbol has an explicit and searchable name."
keywords:
  - "nova"
  - "eslint rule"
  - "default export"
...
- **Anonymous function/class defaults are not flagged** — `export default function () {}` and `export default class {}` have no declaration name to separate, so they are allowed. Only named declarations like `export default function Foo()` are reported.
