...
The `Logger` battery locks down terminal output with environment-aware level gating, scope-able helpers, and <Term title="escape codes defined by the American National Standards Institute" to="/docs/quickstart/terminology#runtime-and-browser">ANSI</Term>-safe formatting so debug chatter stays in dev and production logs stay spotless.
...
- **Colors missing in CI logs.** — Some environments disable <Term title="escape codes defined by the American National Standards Institute" to="/docs/quickstart/terminology#runtime-and-browser">ANSI</Term>. Export `FORCE_COLOR=1` or rely on timestamps/names for identification.
