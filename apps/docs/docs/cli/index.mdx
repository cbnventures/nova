...
Running `nova recipe github` (alias `nova rcp gh`) without a subcommand executes all three GitHub recipes in sequence. Pass `-d` or `--dry-run` to preview the commands without invoking `gh`.
