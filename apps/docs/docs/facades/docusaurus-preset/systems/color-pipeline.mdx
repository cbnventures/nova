...
description: "Convert a single hex color through HSL into an 11-level shade scale used as CSS custom properties across every preset layer."
sidebar_position: 3
keywords:
  - "nova"
  - "docusaurus facades"
...
Each preset declares six color categories, each as a `{ light, dark }` hex pair. The two brand colors — `primary` and `secondary` — pass through a generation pipeline to produce 11-level shade scales.

The remaining four (`text`, `border`, `warning`, `danger`) are used as single values plus a handful of derived `color-mix` tokens.
