...
id: "bare-mode"
title: "Bare Mode"
description: "Strip the site chrome from any Docusaurus page with a query parameter so it can be embedded in a mobile webview or iframe."
sidebar_position: 5
keywords:
  - "nova"
  - "docusaurus facades"
...
- **Full git history** — build from a full-history checkout. Shallow <Terminology title="continuous integration and deployment" to="/docs/quickstart/terminology#builds-and-tooling" color={true}>CI</Terminology> checkouts (`fetch-depth: 1`) make every page report the date of the latest commit, so set `fetch-depth: 0` in your checkout step.
