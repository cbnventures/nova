...
- **Strings passed to `new RegExp()` are not flagged** — The first argument to `new RegExp(...)` is skipped, since `${...}` there is valid regex syntax (quantifiers/literals), not a missing-backtick mistake.
