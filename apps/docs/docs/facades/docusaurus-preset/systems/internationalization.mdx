...
description: "Reconcile and measure the Docusaurus translation tree with theme-nova i18n sync, check, and coverage while preserving every real translation."
keywords:
  - "nova"
  - "docusaurus facades"
...
`theme-nova i18n` reconciles the tree in both directions: it seeds new keys, prunes redundant copies, and surfaces orphaned keys for review before anything is deleted.

Three subcommands cover reconciliation and completeness:
...
- `theme-nova i18n coverage` — report how much of each configured locale is translated and optionally enforce a minimum percentage.
...
**User translations always win.** Reconciliation never overwrites a message that differs from its resolved default.

A value that matches the default exactly is a redundant copy and can be pruned; any other value is treated as a deliberate translation or override and is kept verbatim.
...
The comparison is an exact match on the message string — any difference in whitespace, plural form, or placeholder order counts as an override and is kept. Reconciliation only ever drops a copy that is byte-for-byte identical to its default, so a real translation is never mistaken for a redundant one.
:::
...
- **Non-interactive (<Terminology title="continuous integration and deployment" to="/docs/quickstart/terminology#builds-and-tooling" color={true}>CI</Terminology>, no TTY)** — `sync` blocks, writes nothing, and exits `1`, printing every orphan so the drift is visible in the log.
...
<Tabs groupId="i18n-invocation">
  ...
  <TabItem value="direct" label="Direct">
    ...
    ```bash
    theme-nova i18n coverage [options]
    ```
...
Nova reads the active locale's Docusaurus `direction`. When it is `rtl`, the preset automatically appends its bundled RTL CSS transform to the PostCSS pipeline for that locale. Left-to-right locales do not load the transform, and there is no separate Nova preset option to maintain.
...
Write logical CSS properties such as `margin-inline-start` when possible. The automatic transform covers directional declarations from Nova and third-party styles during the RTL locale build.

## Replacing `write-translations`

`theme-nova i18n sync` is a complete replacement for `docusaurus write-translations` on Nova sites. It performs the same extraction and seeding, then adds the pruning and orphan-gating that `write-translations` lacks.

There is no separate post-prune step to run afterward — the single `sync` command both writes new keys and removes the redundant ones in one pass. Point your `i18n` scripts at `theme-nova i18n sync` and drop `docusaurus write-translations`.
