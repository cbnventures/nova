...
description: "Generate agent convention files for the AI tools you select, like Claude Code and Codex, so agents follow project-specific rules."
keywords:
  - "nova"
  - "nova generate"
  - "agent conventions"
...
The generator only emits files for the AI tools your repository selects. Selection lives in the top-level `agents` field of `nova.config.json`:
...
Each id maps to a single tool file:

| Agent id      | File emitted |
|---------------|--------------|
| `claude-code` | `CLAUDE.md`  |
| `codex`       | `AGENTS.md`  |

The shared files — `VISION.md`, `PROJECT_RULES.md`, and every `conventions/*.md` — are tool-agnostic and are written whenever at least one agent is selected.

You can edit `agents` by hand or manage it interactively through the **AI Tools** category of [`nova utility initialize`](/docs/cli/utilities/initialize), which requires at least one selection.

:::warning

When `agents` is absent or empty, the generator writes nothing and warns `No AI agents selected. Nothing to generate.` Select at least one tool before running the command.
...
- Emits only the tool files you select — `CLAUDE.md` for Claude Code, `AGENTS.md` for Codex — alongside the shared `VISION.md`, `PROJECT_RULES.md`, and `conventions/*.md`.
...
The exact set depends on which agents you select. Each selected tool contributes its own file:

| File        | Written when `agents` includes | Purpose                          |
|-------------|--------------------------------|----------------------------------|
| `CLAUDE.md` | `claude-code`                  | Claude Code project instructions |
| `AGENTS.md` | `codex`                        | Codex project instructions       |

The following shared files are written whenever at least one agent is selected:
...
The language convention files (such as `conventions/typescript.md`) instruct authors to tag new or changed API with `@since UNRELEASED` rather than a hard-coded version number. The real version is stamped automatically when you run `nova utility changelog --release` at release time.

:::info

Never write a concrete version in `@since` or `@deprecated` by hand. Using `@since UNRELEASED` keeps the tag accurate until the release tooling fills it in.
...
Copy the prompt below and paste it into your AI coding assistant (e.g., Claude Code, Codex, etc.). The agent will guide you through 80 questions.
