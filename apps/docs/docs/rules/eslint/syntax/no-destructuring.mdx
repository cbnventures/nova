...
<Tabs groupId="no-destructuring-examples">
  ...
  <TabItem value="incorrect" label="Incorrect">
    ...
    // for...of — destructured loop variable.
...
- **Destructuring in a callback to a non-array-method (e.g. `.then()`, `.addEventListener()`, a custom function) is not flagged** — `callbackParams` only covers a fixed list of array methods, and `functionParams` deliberately skips any function passed as a call argument. There is currently no option that flags these.
