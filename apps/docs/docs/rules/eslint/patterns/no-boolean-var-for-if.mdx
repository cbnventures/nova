...
description: "Disallow extracting a multi-condition boolean into a single-use variable when the condition can be inlined in the next if-statement."
keywords:
  - "nova"
  - "eslint rule"
  - "boolean variable"
...
- **Warning does not fire on a multi-variable declaration** — The rule only checks single-declarator `const` statements. A declaration that declares more than one variable (e.g. `const a = x && y, b = 2;`) is not flagged.
