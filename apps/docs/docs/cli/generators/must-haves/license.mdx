...
description: "Generate a LICENSE file populated with license type, author, and year information from nova.config.json supporting MIT, Apache, and more."
keywords:
  - "nova"
  - "nova generate"
  - "license"
...
| `project.license`      | SPDX license identifier. Required; the command exits with an error if it is not set. |
...
| `<workspace>/LICENSE` | A copy of the same license placed in each consumer-facing workspace (roles `app`, `package`, `tool`, and `config`), excluding the root workspace. |
