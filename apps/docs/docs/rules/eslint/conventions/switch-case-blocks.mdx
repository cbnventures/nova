---
id: "switch-case-blocks"
title: "Switch Case Blocks"
description: "Require every non-empty switch case to wrap its body in a block so declarations stay scoped and branches stay independent."
keywords:
  - "nova"
  - "eslint rule"
  - "switch case"
  - "block scope"
  - "code style"
  - "linting"
tags:
  - "rules"
  - "eslint"
  - "code style"
---

import { Terminology } from '@cbnventures/docusaurus-preset-nova/blocks';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

Enforce block-wrapped switch cases so each branch is self-contained and declarations never leak across cases.

## Summary

The `switch-case-blocks` rule requires every non-empty `switch` case to wrap its body in a block (`{ }`). Empty cases used for intentional fallthrough are allowed.

## Why Use This Rule?

1. Prevents accidental variable leaks between cases that share the same function scope.
2. Makes each case self-contained so reordering or removing one won't break another.
3. Gives a consistent visual boundary that's easier to scan.

## Examples

<Tabs groupId="switch-case-blocks-examples">
  <TabItem value="correct" label="Correct" default>

    ```ts
    switch (action) {
      case 'start': {
        const id = getId();
        run(id);
        break;
      }

      case 'stop': {
        cleanup();
        break;
      }

      // Empty cases for intentional fallthrough are fine.
      case 'pause':
      case 'resume':
      default: {
        log('unhandled action');
        break;
      }
    }
    ```

  </TabItem>
  <TabItem value="incorrect" label="Incorrect">

    ```ts
    switch (action) {
      case 'start':
        const id = getId();
        run(id);
        break;

      case 'stop':
        cleanup();
        break;

      case 'pause':
      case 'resume':
      default:
        log('unhandled action');
        break;
    }
    ```

  </TabItem>
</Tabs>

## Configuration

<Tabs groupId="switch-case-blocks-config">
  <TabItem value="default" label="Default" default>

    Enable the rule in your ESLint <Terminology title="single-file config" to="/docs/quickstart/terminology#builds-and-tooling" color={true}>flat config</Terminology>:

    ```ts
    import { SwitchCaseBlocks } from '@cbnventures/nova/rules/eslint';

    export default [
      {
        plugins: {
          '@cbnventures/nova': {
            rules: {
              'switch-case-blocks': SwitchCaseBlocks.rule,
            },
          },
        },
        rules: {
          '@cbnventures/nova/switch-case-blocks': ['error'],
        },
      },
    ];
    ```

  </TabItem>
  <TabItem value="skip-default-case" label="Skip Default Case">

    Allow switch statements without a `default` case:

    ```ts
    import { SwitchCaseBlocks } from '@cbnventures/nova/rules/eslint';

    export default [
      {
        plugins: {
          '@cbnventures/nova': {
            rules: {
              'switch-case-blocks': SwitchCaseBlocks.rule,
            },
          },
        },
        rules: {
          '@cbnventures/nova/switch-case-blocks': ['error', {
            requireDefault: false,
          }],
        },
      },
    ];
    ```

  </TabItem>
  <TabItem value="skip-files" label="Skip Files">

    Exclude specific files or patterns with the `ignoreFiles` option:

    ```ts
    import { SwitchCaseBlocks } from '@cbnventures/nova/rules/eslint';

    export default [
      {
        plugins: {
          '@cbnventures/nova': {
            rules: {
              'switch-case-blocks': SwitchCaseBlocks.rule,
            },
          },
        },
        rules: {
          '@cbnventures/nova/switch-case-blocks': ['error', {
            ignoreFiles: [
              '*.test.ts',
            ],
          }],
        },
      },
    ];
    ```

  </TabItem>
</Tabs>

## Options

:::warning
`ignoreFiles` is an escape hatch for files where this rule genuinely does not apply. It is not intended for routine use.
:::

| Option           | Type       | Default | Description                                                                                                                               |
|------------------|------------|---------|-------------------------------------------------------------------------------------------------------------------------------------------|
| `ignoreFiles`    | `string[]` | `[]`    | File names to skip. Supports a leading-`*` suffix (`*.test.ts`), an exact path, or a trailing path segment; `**` globs are not supported. |
| `requireDefault` | `boolean`  | `true`  | Require a `default` case in switch blocks.                                                                                                |

## Autofix

Autofix is not available for this rule. Flagged code must be updated manually.

## Troubleshooting

- **False positive on an empty case** — Empty cases (no statements at all) are allowed for intentional fallthrough. If the case has any statement, it needs a block.
- **Still flagged after adding a block** — The block must be the case's only statement. A case with a block plus a sibling statement outside it (e.g. a trailing `break` after the closing brace) is still reported; move every statement, including `break`, inside the braces.
- **Conflicts with another formatter** — Some formatters strip braces from single-statement cases. Configure the formatter to preserve block statements inside switch cases.
