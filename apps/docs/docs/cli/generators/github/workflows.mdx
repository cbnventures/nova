...
| `check-sponsor-gated-issues` | Gate issues by GitHub sponsor status          | Requires a `PERSONAL_ACCESS_TOKEN` secret plus several `var`-format settings (see [check-sponsor-gated-issues Variables](#check-sponsor-gated-issues-variables)). |
...
| Type                          | Description                                         | Secrets / Variables                                                                                                                                                                                                                                                                                                                                     |
|-------------------------------|-----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| `github-action`               | Publish a JavaScript GitHub Action via orphan tags  | None to configure on GitHub. Its `ACTION_ENTRY_POINT`, `ACTION_OUTPUT_PATH`, `ACTION_YML_PATH`, and `RELEASE_BRANCH_NAME` are `literal` settings (see [Publishing GitHub Actions](#publishing-github-actions)).                                                                                                                                         |
...
| `aws-amplify-nextjs`          | Deploy a Next.js app to AWS Amplify                 | `AWS_ACCESS_KEY_ID` (`secret`) - AWS access key ID for Amplify deployments. `AWS_SECRET_ACCESS_KEY` (`secret`) - AWS secret access key for Amplify deployments. `AMPLIFY_APP_ID` (`var`) - Amplify application ID. `AMPLIFY_BRANCH_NAME` (`var`) - Amplify branch name to deploy to. `AWS_REGION` (`var`) - AWS region where the Amplify app is hosted. |
...
8. Each target's `type` and `workingDir` pair is unique within the entry.
