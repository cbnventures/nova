...
- **Manage existing variables** — Opens a menu where you can add, edit, or delete environment variables in both files. Template variables (shipped with Nova) can be edited but cannot be deleted. Changes are saved to both `.env` and `.env.sample` simultaneously.
