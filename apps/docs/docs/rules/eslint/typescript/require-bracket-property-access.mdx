...
description: "Require bracket notation instead of dot notation for property access on project-defined plain objects to keep access patterns consistent."
keywords:
  - "nova"
  - "eslint rule"
  - "typescript"
...
<Tabs groupId="require-bracket-property-access-examples">
  <TabItem value="correct" label="Correct" default>
    ...
    // Dot notation is allowed for class instances.
    class Service {
      url = '';
    }
...
- **Error fires on a class instance property** — Class instance members accessed via a typed instance variable are excluded. If the rule still fires, check that the type is declared as a `class` and not a plain `type` or `interface`.
...
- **Rule does nothing in `.js`/`.mjs` files** — The rule relies on TypeScript type information. Files parsed without TypeScript parser services (plain `.js`/`.mjs`, or any file not covered by a typed ESLint config) are skipped entirely.
