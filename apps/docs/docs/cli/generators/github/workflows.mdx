...
id: "workflows"
title: "Workflows"
description: "Generate GitHub Actions workflow files from nova.config.json for CI/CD automation including checks, publishing, and issue management."
keywords:
  - "nova"
  - "nova generate"
  - "github"
...
- **Corepack package manager** — A project with a `publish` workflow must declare an exact root `packageManager` value for npm, pnpm, or Yarn. Ranges, partial versions, missing values, and unsupported clients are rejected before any workflow file changes.
...
The project root's `packageManager` field selects how generated `publish` workflows install dependencies and run project-owned tools. Nova passes the client through the entire workflow instead of translating every project to npm.

| Root descriptor        | Install step   | Local executable   | Workspace scripts without Turbo           |
|------------------------|----------------|--------------------|-------------------------------------------|
| `npm@<exact-version>`  | `npm install`  | `npx <tool>`       | `npm run <script> -w <workspace>`         |
| `pnpm@<exact-version>` | `pnpm install` | `pnpm exec <tool>` | `pnpm --filter=<workspace> run <script>`  |
| `yarn@<exact-version>` | `yarn install` | `yarn exec <tool>` | `yarn workspace <workspace> run <script>` |

Each install job first runs `corepack enable <client>`. Corepack then reads the full descriptor from the checked-out root `package.json`, so the workflow uses the pinned version instead of installing a floating `latest` release. Turbo projects use the same client to execute `turbo`.

This selection governs project tooling; it does not rename deploy destinations. The `npm` and `github-packages` targets still use the npm publishing protocol and commands even when pnpm or Yarn manages the repository.
...
This template declares the following variables. `GITHUB_TOKEN` is supplied automatically, so the only `secret` you create manually is `PERSONAL_ACCESS_TOKEN`.

The remaining values are `var`-format, not `literal` - they resolve to `${{ vars.NAME }}` references and are configured as GitHub repository variables.
...
The three `workflow-run-*` triggers start a new publish workflow after a named upstream workflow finishes. Choose the variant that matches the upstream result you accept:

| Trigger                | When the downstream jobs run                                  |
|------------------------|---------------------------------------------------------------|
| `workflow-run-success` | After an upstream run finishes successfully.                  |
| `workflow-run-failure` | After an upstream run finishes with a failure.                |
| `workflow-run-any`     | After an upstream run finishes, regardless of its conclusion. |

Declare one of these triggers in object form. Its `workflows` array contains sibling `<template>-<name>` keys from the same `workflows` config:
...
Nova turns `publish-core` into the generated upstream workflow's display name. When that workflow finishes successfully, GitHub starts `publish-preset` as a separate run.

```mermaid
flowchart LR
  core["publish-core finishes successfully"] --> preset["publish-preset starts"]
```

:::warning Multiple names mean “any,” not “all”

GitHub treats multiple entries in `workflows` as alternatives. This configuration starts `publish-preset` when **either** `publish-core` **or** `publish-cli` finishes successfully; it does not wait for both. If both finish, each completion can start its own downstream run.

```json
{
  "name": "workflow-run-success",
  "workflows": ["publish-core", "publish-cli"]
}
```

If several packages must publish in one coordinated run, keep them in one `publish` workflow and use [`deploy[].after`](#deploy-fields) for destination ordering.
:::

Each referenced key must resolve to a sibling workflow, and Nova rejects circular references. The downstream run starts fresh; Nova does not automatically carry build files or artifacts over from the upstream run.

GitHub also requires the downstream workflow file to exist on the repository's default branch and limits `workflow_run` chains to three levels. See GitHub's [`workflow_run` documentation](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#workflow_run) before designing a longer chain.

## Environment Variables and Secrets

A publish workflow reads workspace environment values declared under [`environment.workspaces`](/docs/quickstart/nova-config#environment). Every value has a required `reach`, which tells Nova where the value is allowed to travel:

| `reach`   | Local `.env` | GitHub storage | CI build environment | Deployed runtime |
|-----------|--------------|----------------|----------------------|------------------|
| `local`   | Yes          | No             | No                   | No               |
| `managed` | Yes, blank   | Yes            | No                   | No               |
| `build`   | Yes          | Yes            | Yes                  | No               |
| `runtime` | Yes          | Yes            | No                   | Yes              |

For `managed`, `build`, and `runtime`, the independent `secret` flag controls storage: `secret: false` uses a GitHub Variable, while `secret: true` uses a GitHub Secret. A `local` value never leaves the developer's machine, so it does not accept `secret` and does not need a namespace prefix.

A build value exists only while the build runs on the <Terminology title="continuous integration and deployment" to="/docs/quickstart/terminology#builds-and-tooling" color={true}>CI</Terminology> runner. A Variable that application code inlines (for example, `import.meta.env.PUBLIC_SITE_KEY`) can become part of the output; a Secret used for authentication can be consumed without being emitted. **"Build" means "available to the build process," not automatically "shipped to users."** Whether a value is exposed in client output depends on how the application uses it.

A runtime value is reconciled into a running server's environment and read by reference (`env.X` or `process.env.X`). A managed value is tracked locally and on GitHub but is intentionally delivered to neither destination.
...
Every workspace value with `reach: "build"` is written into the build `.env` before `check` / `build`. Nova reads it from its GitHub **Variable** (`${{ vars.<name> }}`) when `secret: false`, or its GitHub **Secret** (`${{ secrets.<name> }}`) when `secret: true`.

A `secret: false` Variable falls back to its `defaultValue` when the Variable is unset. Deploy credentials and workflow-config keys never reach the build.

The [Setup report](#setup-instructions) lists every GitHub Variable and Secret to create.

### Runtime Environment Sync

On each deploy, Nova reconciles every workspace value with `reach: "runtime"` into the runtime store of **every server-bearing target** in the workspace's `deploy[]`. `secret` picks the channel: a `secret: true` value is stored **encrypted**, while a `secret: false` value is stored as a **plaintext** runtime variable.

- **Upsert** — Every declared runtime value is pushed to the destination under its prefixed name, and the running app reads it as `env.NAME` on Workers or `process.env.NAME` on Vercel.
...
Nova has full authority over each server's runtime env — removing a `reach: "runtime"` value from the config deletes it from every server on the next deploy, and any value set out-of-band is reconciled away. The sync runs unattended in <Terminology title="continuous integration and deployment" to="/docs/quickstart/terminology#builds-and-tooling" color={true}>CI</Terminology>, so there is no interactive confirm — the deploy log records every change.
:::
...
When at least one valid `publish` entry exists, the generator validates the root `packageManager` first. If that project-level setting is missing or invalid, the whole generation run stops before writing, backing up, or deleting workflow files.

The generator also checks each configured entry:
...
6. Each `build` scope is a registered workspace, and no workspace still carries the retired `dotenv` block — that model moved to the top-level `environment` block classified by `reach` and, where applicable, `secret`, so an old-shape config is rejected with a migration hint.
...
Each entry-level failure emits a diagnostic and skips the affected workflow, while other valid workflows still build. The project-level package manager guard is the exception because every publish entry shares it.

### File Generation

Each valid entry produces one YAML file under `.github/workflows/`. The generator builds the workflow blueprint from your config — merging the resolved triggers, jobs, deploy destinations, and selected Corepack client — resolves variable placeholders from the template metadata and the entry's `with`, then serializes the blueprint to YAML.
...
After all files are generated, the generator prints a Setup report of the secrets and variables to configure on GitHub. This covers every non-automatic template `secret` and `var`, plus every `reach: "build"` workspace value each build scope reads — listed under its prefixed GitHub name as a Variable (`secret: false`) or a Secret (`secret: true`).
...
The `github-action` destination distributes a JavaScript GitHub Action by maintaining an orphan `releases` branch whose commits contain only the runtime files needed by consumers. Each release is tagged with its semver (e.g., `v2.1.0`), and the floating major tag (`v2`) is force-moved to the latest release commit.

Older frozen major tags (e.g., `v1`) are never touched.
...
1. The base `build` job runs the workspace's build command through the root `packageManager` client (executing Turbo when a `turbo.json` is present, otherwise using the client's workspace command), producing output in the workspace's `build/` directory.
...
8. A SLSA build-provenance attestation is emitted on the resolved entry script (`ACTION_OUTPUT_PATH`/`ACTION_ENTRY_POINT`) via `actions/attest@v4`.
...
Each action gets its own publish workflow with a distinct `RELEASE_BRANCH_NAME`, and the GitHub Releases for each action use a uniquely-prefixed tag (e.g., `primary-v1.2.3`, `secondary-v1.0.0`). Nova's force-retag step is prefix-aware: the floating major derivation matches an optional alphanumeric prefix before the `v` (so `primary-v1.2.3` → `primary-v1`, `v2.1.3` → `v2`).

Consumers reference each action via its prefixed tag: `uses: owner/repo@primary-v1`, `uses: owner/repo@secondary-v1`.
...
The force-retag steps require permission to overwrite existing tags. If the consuming repo has [tag protection rules](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-security/managing-repository-settings/configuring-tag-protection-rules) covering the `v*` pattern, the workflow's default `GITHUB_TOKEN` will be denied — even with `contents: write` permission.

Either disable tag protection for the patterns you ship, or replace `GITHUB_TOKEN` with a [GitHub App token](https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/about-authentication-with-a-github-app) that has the required maintain-permission scope.
:::
