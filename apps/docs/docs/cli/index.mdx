...
description: "Generate files, apply recipes, scaffold projects, and run utilities with explicit Nova CLI commands and predictable behavior."
sidebar_position: 1
keywords:
  - "nova cli"
  - "generate files"
...
flowchart TD
    ...
    subgraph Recipes
        direction LR
        RCP --> GHR[github] --> GHR1[sync-features / sync-identity / sync-policies]
        RCP --> PKG[package-json] --> PKG1[cleanup / normalize-* / sync-*]
        RCP --> MISC[miscellaneous] --> MISC1[fix-markdown-tables]
    end
...
Apply **scripted edits** that automate routine maintenance, using values from <Terminology title="Nova config" to="/docs/quickstart/terminology#nova-concepts" color={true}>`nova.config.json`</Terminology> when the recipe needs configured project data.
...
| Subcommand                                                | Shorthand   | What It Does                                                                                            |
|-----------------------------------------------------------|-------------|---------------------------------------------------------------------------------------------------------|
| _(none)_                                                  | —           | Run all GitHub recipes (`sync-features`, `sync-identity`, `sync-policies`) for the current repository.  |
| [`sync-features`](/docs/cli/recipes/github/sync-features) | `sync-feat` | Sync repository feature flags (issues, wiki, projects, discussions) to [GitHub](https://github.com/).   |
| [`sync-identity`](/docs/cli/recipes/github/sync-identity) | `sync-id`   | Sync repository identity (description, homepage URL, topics) to [GitHub](https://github.com/).          |
| [`sync-policies`](/docs/cli/recipes/github/sync-policies) | `sync-pol`  | Sync repository policies (visibility, merge methods, branch settings) to [GitHub](https://github.com/). |

Running `nova recipe github` (alias `nova rcp gh`) without a subcommand executes all three GitHub recipes in sequence. Pass `-d` or `--dry-run` to run without executing any GitHub commands.
...
| [`sync-identity`](/docs/cli/recipes/package-json/sync-identity)                   | `sync-id`   | Sync name, version, description, keywords, displayName, and license to workspace `package.json` files. |
...
#### Miscellaneous
_Category: `miscellaneous` · Alias: `misc`_

| Subcommand                                                                   | Shorthand | What It Does                                                |
|------------------------------------------------------------------------------|-----------|-------------------------------------------------------------|
| _(none)_                                                                     | —         | Run every miscellaneous recipe for the current repository.  |
| [`fix-markdown-tables`](/docs/cli/recipes/miscellaneous/fix-markdown-tables) | —         | Format Markdown and MDX tables throughout the project root. |

Running `nova recipe miscellaneous` (alias `nova rcp misc`) without a subcommand runs every miscellaneous recipe. Currently, that means `fix-markdown-tables`.
