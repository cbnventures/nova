...
id: "theme-config"
title: "Theme Config"
description: "Configure the Nova Docusaurus theme with this full reference covering site metadata, navbar, footer, docs, blog, and color mode."
sidebar_position: 2
keywords:
  - "nova"
  - "docusaurus facades"
...
| `items`     | `NavbarItem[] \| undefined` | Nested items when `type` is `'dropdown'`.                      |
| `docId`     | `string \| undefined`       | Document ID when `type` is `'doc'`.                            |
...
Customize copy on the 404, route-error, and React error-boundary surfaces. Each sub-key is optional; omit a key to use the preset's built-in defaults.

Each leaf value is also optional, so you can override just the fields you care about.
...
| `footer.socialLinks` | `Array<{ icon: string \| IconifyIcon, href: string, label: string }>` | `[]`        | Social media icon links using an Iconify identifier or inline icon definition.                             |
...
### Inline Iconify Objects

Use an Iconify identifier for a registered icon, or provide an inline object when the SVG definition belongs in config. `body` is required; the remaining fields match the Iconify icon geometry and transformations.

| Property | Type      | Required | Description                                      |
|----------|-----------|:--------:|--------------------------------------------------|
| `body`   | `string`  | Yes      | SVG body markup.                                 |
| `left`   | `number`  | No       | Left edge of the view box. Defaults to `0`.      |
| `top`    | `number`  | No       | Top edge of the view box. Defaults to `0`.       |
| `width`  | `number`  | No       | View-box width. Defaults to `16`.                |
| `height` | `number`  | No       | View-box height. Defaults to `16`.               |
| `rotate` | `number`  | No       | Number of 90-degree rotations. Defaults to `0`.  |
| `hFlip`  | `boolean` | No       | Flip the icon horizontally. Defaults to `false`. |
| `vFlip`  | `boolean` | No       | Flip the icon vertically. Defaults to `false`.   |
