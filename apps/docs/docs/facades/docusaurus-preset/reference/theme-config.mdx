...
`blog.layout.heading` and `blog.layout.description` populate the page-level header that `<BlogLayout>` renders above the blog content on the index, archive, and author pages. Swizzle consumers can pass `<BlogLayout showHeader>` to opt into the header on a custom page, or pass a `header` ReactNode prop to override the default heading + description entirely.
:::
