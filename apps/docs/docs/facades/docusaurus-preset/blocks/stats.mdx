...
id: "stats"
title: "Stats"
description: "Display project statistics in a labeled metric grid with optional color accents and per-preset styling on documentation landing pages."
sidebar_position: 3
keywords:
  - "nova"
  - "docusaurus facades"
...
The Stats block renders a `<section>` containing a grid of value–label pairs. Each item can be color-coded using the `color` property to draw attention with the preset's primary or accent color.
...
import { Stats } from '@cbnventures/docusaurus-preset-nova/blocks';
...
| `className`   | `string \| undefined`                                                    | `undefined` | Additional class names merged with the base `nova-stats` class. |
