...
description: "Render Apple App Store and Google Play download buttons with per-preset styling and configurable download link targets on landing pages."
sidebar_position: 7
keywords:
  - "nova"
  - "docusaurus facades"
...
The App Market Download block renders a `<section>` with one or two download buttons. Each button links to the respective app store.

The block renders nothing if neither `appStoreUrl` nor `googlePlayUrl` is provided.
...
import { AppMarketDownload } from '@cbnventures/docusaurus-preset-nova/blocks';
...
| `className`     | `string \| undefined`        | `undefined` | Additional class names merged with the base `nova-app-market-download` class. |
| `style`         | `CSSProperties \| undefined` | `undefined` | Inline style overrides applied to the root element.                           |

## Troubleshooting

- **Nothing rendering** — The block returns nothing if both `appStoreUrl` and `googlePlayUrl` are `undefined`. Set at least one URL.
