...
<Tabs groupId="bootstrap-output">
  <TabItem value="file" label="First match" default>

    ```text
    /Users/you/.config/my-app
    ```

    `resolveFileDir` returns the first matching directory, or `undefined` when no location contains the file.

  </TabItem>
  <TabItem value="files" label="All matches">
    ...
    `resolveFileDirs` returns every distinct match in search order, or an empty array when none match.

  </TabItem>
  <TabItem value="env" label="Environment load">

    ```text
    process.env.API_URL === "https://api.example.com"
    ```

    `loadEnv` returns `undefined` after filling only keys that were not already present in `process.env`.
