...
description: "Disallow await inside loops so sequential async calls that could be parallelized with Promise.all are caught early during linting."
keywords:
  - "nova"
  - "eslint rule"
  - "await in loop"
...
- **Await inside a nested function in a loop is not flagged** — The rule stops searching at function boundaries (function declarations, function expressions, and arrow functions). An `await` inside a callback or inner async function declared within a loop is treated as a new async scope and is intentionally not reported.
