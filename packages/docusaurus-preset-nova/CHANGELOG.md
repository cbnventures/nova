...
### UPDATED
- The mobile docs sidebar overlay and the table-of-contents overlay no longer close themselves when a link inside them is tapped or when the page navigates. They now stay open until you dismiss them explicitly with the close button, the Escape key, or a tap on the backdrop, so you can move between several entries without the overlay collapsing after each tap.
- Blog post reading time is now pluralized through Docusaurus's plural-form selection. An English one-minute post reads `One min read` instead of `1 min read`, and every locale renders the reading-time label with its own plural rules rather than a single fixed form.

### FIXED
- Icon rendering no longer requires the consuming site to declare "@iconify/react" itself. The preset now supplies the Iconify runtime it renders with through its own package export, so the generated icon-registration module resolves the runtime from the preset instead of from the site's own dependencies. A fresh install renders icons without the site depending on its own copy, and sites that added "@iconify/react" solely to satisfy the generated module can remove it.
- The live code-block editor now shows the author's own code for every supported language, not just TSX (React). It previously wrote the snippet into a hardcoded `/App.tsx`, which is the entry file only for the `react-ts` template, so JavaScript, TypeScript, JSX, Vue, Svelte, and Angular blocks rendered their template's default output instead of the authored code. The editor now writes the code into the entry file that Sandpack reports for the resolved template, so each language's live preview reflects the snippet on the page.
- The navbar now highlights the doc navbar item that matches the current page instead of always highlighting the first one. When several doc-type items came from the same docs plugin, each scored its active state against the shared plugin base path, so the first item won the tie and received `aria-current` no matter which doc was open. Each item now scores against its own resolved doc path, so the item whose path best matches the current URL is marked active.
- Doc pages whose git metadata is unavailable (untracked files, shallow CI checkouts) no longer render an empty `<footer class="nova-doc-footer">` element. Docusaurus emits `null` rather than `undefined` for missing last-updated data, and the footer's display check treated `null` as present while the render guards treated it as absent; both the doc footer and the standalone MDX page edit-meta row now treat `null` as absent consistently.
- Client-side documentation search no longer breaks when a query contains lunr query-syntax characters. Queries such as a time (`9:00`), a field expression (`key:value`), or a pasted URL previously reached lunr's parser unguarded and threw an uncaught parse error, silently killing search with no feedback. The search worker now runs its exact, wildcard, and fuzzy passes inside a guard and returns no results for such queries instead of throwing.
- The content-footer share button's copy-link control no longer throws in insecure contexts. Its click handler called `navigator.clipboard.writeText` unconditionally, so on plain HTTP or a LAN IP address, where `navigator.clipboard` is undefined, clicking the copy-link button raised an uncaught error. The handler now guards the Clipboard API the same way the code-block copy control does, doing nothing when it is unavailable and showing the copied confirmation only when the write succeeds.
- A standalone MDX page that sets `hide_table_of_contents: true` in its frontmatter now actually hides the table of contents. The flag was compared against the string `'true'` while its value is a boolean, so the check was always true and the table of contents rendered regardless of the setting. The comparison now reads the boolean, so the documented flag takes effect.
- A navbar that includes a `{ type: 'docsVersionDropdown' }` item now renders instead of crashing the production build. The `NavbarItem` component registry omitted the standard `docsVersionDropdown` type, so Docusaurus threw `No NavbarItem component found` for that item and failed the static build on every page where the navbar appeared. The registry now maps `docsVersionDropdown` to its dropdown component alongside the other supported navbar item types.

### ADDED
- Any docs or MDX page can now be embedded without the site chrome by appending `?docusaurus-data-bare=true` to its URL, with no frontmatter or configuration required. An inline pre-paint script maps `docusaurus-data-*` query parameters to `data-*` attributes on the document element before hydration, and a new shared embed stylesheet keyed off `html[data-bare="true"]` hides the navbar, footer, announcement bar, back-to-top button, skip-to-content link, docs sidebar, breadcrumbs, paginator, table of contents, tags, share buttons, and edit link, while keeping the page title, content, and last-updated date and stretching the content column to a centered full-width reading column. Pages always render with the full chrome by default, and removing the parameter restores it.
- Adds the `theme-nova i18n coverage` command, which reports per-locale translation coverage over both `code.json` strings and Markdown content (docs, blog, and pages). Coverage scores each locale against the union of keys any locale translates, so brand and technical tokens left English in every locale surface as `English-only` rather than counting as gaps; the actionable `To translate` bucket is strings translated in another locale but English here, plus content files with no translated copy. `--min-coverage <percent>` exits 1 when any locale falls below the threshold for continuous integration, `--gaps` lists the exact missing strings and files per locale, and `--locale` narrows the report while still scoring against every configured locale.
- The theme-nova translation registry now ships bundles for all 33 remaining Docusaurus-supported locales — Bulgarian, Bengali, Czech, Danish, German, Spanish, Estonian, Persian, Filipino, French, Hebrew, Hindi, Hungarian, Indonesian, Icelandic, Italian, Japanese, Korean, Norwegian Bokmål, Dutch, Polish, Brazilian Portuguese, European Portuguese, Russian, Slovenian, Serbian, Swedish, Turkmen, Turkish, Ukrainian, Urdu, and Vietnamese, plus Traditional Chinese — bringing full coverage to 35 locales alongside the existing Arabic and Simplified Chinese. Every bundle mirrors the English key registry one-to-one, preserves message placeholders, and carries the exact plural form count each locale's plural rules require, so sites configured for any Docusaurus locale get a fully translated Nova theme out of the box.
- Added a `theme-nova` binary that reconciles the Docusaurus translation tree. `theme-nova i18n sync` regenerates and reconciles `code.json` and the `docusaurus-theme-nova/<area>.json` bundles: it seeds new source keys, drops the redundant `theme.*` and area copies whose message equals the extracted source default or the localized theme-translations default, and preserves every real translation or override verbatim. `theme-nova i18n check` computes the same plan without writing and exits non-zero on drift so CI can enforce a reconciled tree. Orphaned keys whose source string was removed are never dropped silently: an interactive run prompts per file, a non-interactive run blocks and writes nothing, and `--delete-defunct` opts into removing them. It replaces `docusaurus write-translations` for Nova sites by folding extraction, pruning, and orphan gating into one command, so no separate post-prune wrapper is needed.
