...
The Terminology block renders an `<abbr>` (or `<Link>` when `to` is provided) with `aria-label` and `title` attributes set to the expanded abbreviation. On hover, browsers display the tooltip natively.
