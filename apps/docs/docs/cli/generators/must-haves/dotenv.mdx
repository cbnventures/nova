...
id: "dotenv"
title: "Dotenv (.env)"
description: "Generate .env and .env.sample files for managing local environment secrets with per-workspace key definitions from nova.config.json."
keywords:
  - "nova"
  - "nova generate"
  - "dotenv"
...
- Generates `.env` and `.env.sample` for every workspace that declares `environment.workspaces` values, so each workspace in a <Terminology title="multi-package repo" to="/docs/quickstart/terminology#git-and-workflow" color={true}>monorepo</Terminology> gets its own files (including the repo root `./` when it is declared).
...
| Field                                     | Description                                                                                                                       |
|-------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------|
| `environment.workspaces.<path>.variables` | The variables appended to that workspace's generated `.env` files. See [Value Fields](/docs/quickstart/nova-config#value-fields). |

This command reads only each variable's `key` and `defaultValue`. A variable may also declare `secret` and `buildOnly`, which this command ignores — the [publish workflow](/docs/cli/generators/github/workflows#environment-variables-and-secrets) uses them to deliver each value to the build or the running server.

A non-secret `defaultValue` seeds `.env.sample`; the `.env` entry is always left blank so real secrets stay out of version control.
