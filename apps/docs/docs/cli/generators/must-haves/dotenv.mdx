...
This command writes every workspace value regardless of its `reach`. It reads `key` and `defaultValue` for the files; the [publish workflow](/docs/cli/generators/github/workflows#environment-variables-and-secrets) uses `reach` and `secret` for GitHub provisioning and CI delivery.
