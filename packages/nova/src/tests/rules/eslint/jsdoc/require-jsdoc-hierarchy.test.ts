...
import type { Tests_Rules_Eslint_Jsdoc_RequireJsdocHierarchy_RuleTester } from '../../../../types/tests/rules/eslint/jsdoc/require-jsdoc-hierarchy.test.d.ts';
...
const ruleTester: Tests_Rules_Eslint_Jsdoc_RequireJsdocHierarchy_RuleTester = new RuleTester({
  languageOptions: {
    ecmaVersion: 2022,
    sourceType: 'module',
  },
  ...
  valid: [
    ...
    // Correct class hierarchy with custom knownNames.
    {
      ...
      options: [{
        ...
        ignoreFiles: [],
        knownNames: { myapp: 'MyApp' },
        stripDirectories: ['types'],
        warnSkippedScripts: true,
      }],
    ...
    // Extracted <script> virtual file is skipped (non-deterministic filename never converges).
    {
      code: [
        '/**',
        ' * Wrong summary.',
      ...
      filename: '/project/src/components/github.astro/1_1.ts',
    },
