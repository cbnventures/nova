...
sidebar_position: 4
description: "Complete reference for every section and field in the nova.config.json configuration file including project, entities, workflows, and workspaces."
keywords:
  - "nova"
  - "nova config"
  - "nova.config.json"
...
| `versionStrategy`    | `string`  | Version strategy for the project: `"semver"` (default) or `"calver"`. Semver uses major.minor.patch with bump types. CalVer uses YYYY.MM.MICRO with automatic date-based versioning. Locked after the first release is recorded in `CHANGELOG.md`. |
