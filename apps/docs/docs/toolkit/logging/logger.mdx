...
The `Logger` battery locks down terminal output with environment-aware level gating, scope-able helpers, and <Terminology title="escape codes defined by the American National Standards Institute" to="/docs/quickstart/terminology#runtime-and-browser" color={true}>ANSI</Terminology>-safe formatting so debug chatter stays in dev and production logs stay spotless.
...
- **Colors missing in CI logs** — Some environments disable <Terminology title="escape codes defined by the American National Standards Institute" to="/docs/quickstart/terminology#runtime-and-browser" color={true}>ANSI</Terminology>. Export `FORCE_COLOR=1` or rely on timestamps/names for identification.
