...
Every environment variable and secret in the repository lives here, classified by a required `secret` flag and grouped by namespace. The [`nova generate must-haves dotenv`](/docs/cli/generators/must-haves/dotenv) command reads `environment.apps` to write each app's `.env` and `.env.sample`, and the [publish workflow](/docs/cli/generators/github/workflows#environment-variables-and-secrets) delivers each value by kind.

| Field       | Type     | Description                                                                                                                            |
|-------------|----------|----------------------------------------------------------------------------------------------------------------------------------------|
| `global`    | `object` | Optional. Account-wide keys (deploy credentials, managed-only secrets) under one shared `prefix`. See [Global Fields](#global-fields). |
| `apps`      | `object` | Optional. Per-app environment values, keyed by the app's workspace path. See [Apps Fields](#apps-fields).                              |
...
|-------------|----------|----------------------------------------------------------------------|
| `prefix`    | `string` | Required. The uppercase namespace joined to every `key`.             |
| `variables` | `array`  | Optional. App environment values. See [Value Fields](#value-fields). |
...
| `buildOnly`    | `boolean` | Required on an app value. `true` delivers it to the build; `false` syncs it to every server-bearing deploy target. See [Environment Variables and Secrets](/docs/cli/generators/github/workflows#environment-variables-and-secrets). |
