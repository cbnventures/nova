...
description: "Require a private tag in JSDoc blocks for private class members to keep documentation consistent with TypeScript access modifiers."
keywords:
  - "nova"
  - "eslint rule"
  - "jsdoc"
...
The fix inserts the `@private` tag immediately before the `@since` tag. If the JSDoc block has no `@since` tag, no automatic fix is applied and the member must be fixed manually.
...
- **Autofix does not insert `@private`** — The fix anchors on `@since`; if the JSDoc block has no `@since` tag, add one or insert `@private` by hand.
