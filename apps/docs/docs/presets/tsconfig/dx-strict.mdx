...
:::info
`allowSyntheticDefaultImports` is kept `true` here on purpose. With `skipLibCheck: false`, TypeScript checks third-party declaration files such as Zod's `.d.cts` types, which use default-import syntax internally, and turning it off would surface errors in dependency types you do not control.
:::
