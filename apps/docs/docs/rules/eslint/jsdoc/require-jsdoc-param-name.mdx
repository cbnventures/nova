...
description: "Require param descriptions in JSDoc blocks to match the parameter name with the first letter capitalized and a trailing period."
keywords:
  - "nova"
  - "eslint rule"
  - "jsdoc"
...
- **Only JSDoc `/** */` comments are checked** — The rule inspects block comments that begin with `/**` (JSDoc). `@param` lines inside a plain `/* */` block comment are skipped.
