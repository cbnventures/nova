...
A publish workflow delivers every app environment value declared under [`environment.apps`](/docs/quickstart/nova-config#environment). Each value carries two independent, required flags — `secret` (storage) and `buildOnly` (delivery) — and Nova routes delivery entirely off `buildOnly`, never off `secret`.
