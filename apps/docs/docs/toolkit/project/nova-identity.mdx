...
- `copyright` is the composed line, present only when both `project.startingYear` and `project.legalName` exist; its notice reflects `project.license` — an SPDX identifier reads `Licensed under the <id> License.`, while `Proprietary` or an absent license reads `All Rights Reserved.`
