 ...
 *
 * Reads and mutates the GitHub Variable and Secret names Nova manages via the `gh`
 * CLI, only ever handling names and existence and never their values, so a missing or an
 * unauthenticated `gh` simply yields an unavailable read.
 *
 * @since 0.21.0
 */
